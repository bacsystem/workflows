...
execution → review → PR — with **no dependency on any external skill plugin**.

Before cys, the repo depended on an external skill plugin in two ways:

1. **Methodology**: plans were designed/written with that plugin's design and
   planning skills. When a user asked those skills naturally, they ended up
   offered that plugin's *sequential* executors (task-by-task orchestration /
   inline execution) — and `parallel-plan-executor` never ran unless explicitly named.
   This happened in a real pilot run on 2026-07-16.
2. **Machinery**: the workflow's implement/review agents ran that plugin's
...
differentiator over that external plugin is the execution engine — this repo's `Workflow`
script runs independent plan tasks **in parallel** via the dependency DAG, while the
external plugin only executes sequentially.
...
| 2 | Skill names (deliberately distinct from the external plugin's) | `cys:guide`, `cys:design`, `cys:plan`, `cys:run`, `cys:check`, `cys:ship` |
...
| 4 | Installation | Formal Claude Code plugin (manifest + own marketplace), installable via `/plugin` — same mechanism other Claude Code plugins use, not hand-copied folders |
| 5 | `cys:ship` origin | Migrated/adapted from the author's existing git-flow skill (github.com/bacsystem/skills/git-flow): `type/description` branches, Conventional Commits, SemVer via `scripts/next-version.sh`, PR template, auto-tag on main. All other skills written from scratch — no external text copied |
...
| 8 | Run-record directory | `.cys/` in the target repo replaces the old external plugin's run-record convention (progress.md ledger, task-N-brief.md, task-N-report.md, review-*.diff, handoff.md — same mechanism, own folder) |
...
| Skill | Function | Replaces | Origin |
|---|---|---|---|
| `cys:guide` | Index: teaches when to use each cys skill | the external plugin's own index skill | New |
| `cys:design` | Idea → approved design spec (one question at a time, 2-3 approaches, spec file + user review gate) | the external plugin's brainstorming skill | New |
| `cys:plan` | Spec → implementation plan with `### Task N:` blocks and `Consumes`/`Produces` (the format `bin/parse-plan.js` already parses; enforce one-line-per-entry, today a known parser limitation) | the external plugin's plan-writing skill | New |
| `cys:run` | Execute the plan in parallel — launches the `parallel-plan-executor` Workflow | the external plugin's sequential task-by-task / inline executors | Engine already exists |
| `cys:check` | Adversarial review / verification conventions used by the workflow's reviewer agents and available standalone | the external plugin's code-review + verification skills | New |
| `cys:ship` | Handoff: staging, Conventional Commit, SemVer bump, PR creation | the external plugin's branch-finishing skill (+ the handoff agent's hand-rolled conventions) | **Migrated from bacsystem/skills git-flow** |

Not carried over (v1): the external plugin's debugging, code-review-receiving,
parallel-agent-dispatching (the workflow *is* that), skill-authoring, and
git-worktree-management skills (the workflow manages worktrees itself).
...
4. **Prompt cleanup** — delete `FIND_SDD_SCRIPTS`; implement/review prompts call the
   scripts by exact path; every old external-plugin run-record path becomes `.cys/`;
   the handoff agent's prompt follows `cys:ship` conventions instead of restating them.
...
- **F1 — Engine independence** (§5 above). Planned with the external plugin *for the
  last time*; executed with our own workflow (dogfooding from day one).
...
- **F3 — Commands + cut the cord**: `/cys` and `/cys-run`; full end-to-end pilot run
  **with only the cys plugin installed** (the independence proof), logged as a pilot
  ...
  an automated plan task (installing/uninstalling a real plugin isn't something
...
- **The independence-proof pilot** (§6, F3 bullet) was **descoped from the automated
  F3 plan**: installing/uninstalling a real plugin and driving a real
  `/plugin marketplace add` + `/plugin install` flow isn't something a
